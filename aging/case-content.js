...
const places=[lesson.place];
screen(5).querySelector('.video-frame').innerHTML=`<div><div style="font-size:56px">${'⌖'}</div><h3 style="color:white">${esc(lesson.place)}</h3><p>${esc(lesson.region)}</p><div class="field-links">${lesson.links.map(([label,url])=>external(label,url)).join('')}</div><p class="micro" style="color:#e4eee7">${esc(lesson.address)}</p><div class="field-links">${places.map(place=>external(place+' 지도','https://map.naver.com/p/search/'+encodeURIComponent(place))).join('')}</div></div>`;
...
screen(7).querySelector('.lead').textContent='고령 인구의 증가가 생활에 미친 영향을 낱말 카드로 설명해 보세요.';
...
const mapFile=['map-korea.svg','map-seoul.svg','map-korea.svg'][caseIndex];
const mapQuery=caseIndex===1?'서울광장':lesson.place;
locationBox.innerHTML=`<h3>사건이 일어난 곳</h3><a href="https://map.naver.com/p/search/${encodeURIComponent(mapQuery)}" target="_blank" rel="noopener noreferrer"><img src="./${mapFile}" alt="대한민국에서 ${esc(lesson.place)} 지역의 대략적인 위치" width="290" height="330"></a><p class="micro">${esc(lesson.region)}</p>${external('원문에서 더 알아보기',lesson.source)}<small class="map-credit">지도 윤곽: Natural Earth · 지역 위치는 대략 표시</small>`;
